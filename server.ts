...
async function startServer() {
  ...
  // Let's allow up to 15MB payloads so that users can upload high quality food quality photos easily!
...
`;
  ...
  // Main support chat router endpoint
    ...
    try {
      ...
      if (!process.env.GOOGLE_API_KEY && !process.env.GEMINI_API_KEY) {
        return res.status(500).json({
          error: "Neither GOOGLE_API_KEY nor GEMINI_API_KEY environment variable is defined. Please verify it in Settings > Secrets or in your .env file.",
        });
      }
  ...
  // Serve static UI / Integrate Vite middleware
